...
function walk(dir) {
  ...
  // Don't index files in the data directory
  if (path.resolve(dir) === path.resolve(__dirname + '/../data')) {
    skipContents = true;
  }

  // Don't do anything in images
  if (path.resolve(dir) === path.resolve(__dirname + '/../data/images')) {
    return Promise.resolve([]);
  }

  // Don't do anything in .git
  if (path.resolve(dir) === path.resolve(__dirname + '/../data/.git')) {
    return Promise.resolve([]);
  }
