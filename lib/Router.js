  ...
  */
  handleRequest(request, response) {
    ...
    if (url === '/main.css') {
      ...
      this.responseFile('public/sw.js');
    } else if (url.match(/^\/user-images\/([a-zA-Z0-9-_ ]+\.([a-z0-9]+))$/)) {
    ...
    } else if (url === '/favicon-144.png') {
      this.responseFile('assets/img/favicon-144.png');
    } else if (url === '/favicon.svg') {
      this.responseFile('assets/img/favicon.svg');
    } else if (url === '/icons.svg') {
      this.responseFile('assets/img/icons.svg');
    } else if (url === '/manifest.json') {
      _this.prepareResponse(200, {'Content-type': 'application/json'}, Templater.render('manifest.nunjucks'));
    } else if (url === '/robots.txt') {
      _this.prepareResponse(200, {'Content-type': 'text/plain'}, 'User-agent: *\nDisallow: /offline');
    } else if (url === '/offline') {
      _this.prepareResponse(200, {'Content-type': 'text/html'}, Templater.render('errors/offline.nunjucks'));
    ...
    } else if (['/about', '/portfolio'].indexOf(url) > -1) {
    ...
    } else if (url.match(/^\/([a-z0-9-]+)\/([a-z0-9-.]+)\/?$/i)) {
