  ...
  */
  handleRequest(request, response) {
    ...
    if (request.url === '/main.css') {
      ...
      this.prepareResponse(200, {'Content-type': 'text/html'}, Templater.render('home.nunjucks'));
  ...
  responseFile(filepath) {
    ...
    fs.readFile(filepath, function(err, contents) {
      ...
      const statusCode = _this.request.headers['if-none-match'] === md5(_this.request.url) ? 304 : 200;

      _this.prepareResponse(statusCode, {
        'Content-type': contentType,
        'Cache-Control': 'max-age=30',
        Etag: md5(_this.request.url)
      }, contents);
    });
  }
