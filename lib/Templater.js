...
markedRenderer.image = function image(href, title, text) {
  ...
  if (title && (title.substr(0, 5) === ':left' || title.substr(0, 6) === ':right' || title.substr(0, 7) === ':centre')) {
    const alignLength = (title.substr(0, 5) === ':left') ? 5 : ((title.substr(0, 5) === ':right') ? 6 : 7);
    alignment = title.substr(1, alignLength - 1);
    title = title.substr(alignLength + 1);
  }
