...
module.exports = function routes(router) {
  ...
  router.match(/^\/(favicon(-144)?.png|favicon.svg|icons.svg)$/, (url, request) => {
    return respond.file('assets/img' + url, request);
  });

  router.match(/^\/manifest.json$/, () => {
    return respond.template('errors/301.nunjucks', {}, {status: 301, Location: '/manifest.webmanifest', 'Content-type': 'text/html'});
  });

  router.match(/^\/manifest.webmanifest$/, () => {
    return respond.template('manifest.nunjucks', {}, {'Content-type': 'application/json'});
  });

  router.match(/^\/robots.txt$/, () => {
    return respond.text('User-agent: *\nDisallow: /offline', {'Content-type': 'text/plain'});
  });

  router.match(/^\/offline$/, () => {
    return respond.template('errors/offline.nunjucks', {}, {'Content-type': 'text/html'});
  });
  ...
  router.match(/^\/$/, () => {
    ...
    return Promise.all([series, posts]).then(function([series, posts]) {
      return respond.template('home.nunjucks', {series, posts}, {'Content-type': 'text/html'});
    });
  });
  ...
  router.match(/^\/(about|portfolio)\/?$/, (url) => {
    ...
    return Post.find({series: null, id: parts[1]}).then(function(post) {
      if (typeof post === 'undefined') return;

      // Show the post
      return respond.template('post.nunjucks', post, {'Content-type': 'text/html'});
    });
  });
  ...
  router.match(/^\/archive\/?$/, () => {
    ...
    return Post.all({
      ...
      return respond.template('archive.nunjucks', {posts}, {'Content-type': 'text/html'});
    });
  });
  ...
  router.match(/^\/([0-9-]+)(?:\/([0-9-]+))?\/?$/i, (url) => {
    ...
    return respond.template('errors/301.nunjucks', {}, {status: 301, Location: '/archive#year-' + parts[1], 'Content-type': 'text/html'});
  });
